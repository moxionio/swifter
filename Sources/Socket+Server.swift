...
extension Socket {
    ...
    static func htonl(_ val: UInt32) -> UInt32 {
        let isLittleEndian = Int(OSHostByteOrder()) == OSLittleEndian
        return isLittleEndian ? _OSSwapInt32(val) : val
    }
    ...
    ///       otherwise - in IPv6.
    public class func tcpSocketForListen(_ port: in_port_t, _ forceIPv4: Bool = false, _ maxPendingConnection: Int32 = SOMAXCONN, _ bindToLocalhost: Bool = false) throws -> Socket {
        ...
        if forceIPv4 {
            ...
            var addr = sockaddr_in(
                ...
                sin_port: port.bigEndian,
                sin_addr: in_addr(s_addr: bindToLocalhost ? htonl(INADDR_LOOPBACK) : htonl(INADDR_ANY)),
                sin_zero:(0, 0, 0, 0, 0, 0, 0, 0))
            #endif
